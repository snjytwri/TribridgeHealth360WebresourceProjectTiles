...
var assignedToFieldName = "tri_assigntoid";
var territoryFieldName = "tri_territoryid";

var SavedQuerySetName = "SavedQuerySet";
var defaultViewName = "MH Caseload by Territory";
var caseloadViewName = "MH Caseload by Territory";
var territoryName = "";

var defaultLayoutXML = "";
var fetchXml;

// FUNCTION: formOnLoad
function form_onload() {
    availableDefaultViewId = Xrm.Page.getControl(assignedToFieldName).getDefaultView();
    // Find the Guid for the Active Contacts View - this will be used as the default if we encounter any error.
    var viewData = findViewbyNameSync(SavedQuerySetName, "SavedQueryId, Name, LayoutXml, FetchXml", caseloadViewName);
    if (viewData != null && viewData[0].SavedQueryId != null && viewData[0].SavedQueryId != emptyGuid)
        availableDefaultViewId = viewData[0].SavedQueryId;
    setLookup(false);
}


function retrieveDefaultLayout() {
    //findViewbyName(SavedQuerySetName, "Name, LayoutXml", defaultViewName, processLayout, asyncError);
    setAssignedToView();
}

function processLayout(results) {
    //defaultLayoutXML = results.d.LayoutXml;
    defaultLayoutXML = results.d.results[0].LayoutXml;
    //setContactLookup(false);
    setAssignedToView();
}
...
function setLookup(resetSelection) {
    var terrCntrl = Xrm.Page.getAttribute(territoryFieldName);
    if (terrCntrl != null) {
        var territory = Xrm.Page.getAttribute(territoryFieldName).getValue();
        if (territory != null && territory[0].id != null && territory[0].id != "") {
            var territoryId = territory[0].id;
            territoryName = territory[0].name;
            ...
            fetchXml = "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='true'>" +
                                "<entity name='systemuser'>" +
                                    ...
                                    "<filter type='or'>" +
                                       "<condition attribute='territoryid' operator='eq' value='" + territoryId + "'  />" +
                                    "</filter>" +
            ...
            // Add Code to See if any Contact exists with one of these numbers
            retrieveRecordUsingFetchXml(fetchXml, processFetchResults, errorHandlerFetch);
            //DEMO_ExecuteCrmSoapPostAction(fetchXml, processFetchResults);
        } else {
            // no territory is selected, reset Lookup View to the default view
            setViewToActive();
        }
    } else {
        // no territory control, reset Lookup View to the default view
        setViewToActive();
    }
}

function setViewToActive() {
    document.getElementById(assignedToFieldName).setAttribute("defaulttype", "2");
    Xrm.Page.getControl(assignedToFieldName).setDefaultView(availableDefaultViewId);
}

function processFetchResults(responseXML) {
    var hasEntities = ContainsEntities(responseXML);
    if (hasEntities == true)
        retrieveDefaultLayout();
    else
        setViewToActive();

}

function setAssignedToView() {
    ...
    var viewDisplayName = "Associated Professional for " + territoryName + "";
    ...
    if (defaultLayoutXML == "") {
        var layoutXml = "<grid name='resultset' object='1' jump='contactid' select='1' icon='1' preview='1'>" +
                            "<row name='result' id='systemuserid'>" +                            
                               ...
                               "<cell name='tri_mhcaseload' width='150' />" +
        ...
        layoutXml = defaultLayoutXML;
    }

    // add the Custom View to the primary contact lookup control      
    Xrm.Page.getControl(assignedToFieldName).addCustomView(viewId, entityName, viewDisplayName, fetchXml, layoutXml, true);
}


function findViewbyName(EntitySetName, AttributeSet, ViewName, async_callback, asyncError_callback) {
    //This function returns a CRM JSON dataset for a single entity record based on the View Name

    //*EntitySetName* - is the name of the CRM entity set (Case Sensitive)
    //*AttributeSet* - is the commaseparated list of attributes/fields to return  (Case Sensitive)
    //*EntityId* - is the Guid for the entity record
    //*async_callback* - is the name of the function to call when returning the result
    ...
    try {
        ...
        var selectString = "?$filter=Name eq '" + ViewName + "'&$select=" + AttributeSet;
        ...
        $.ajax({
            ...
            success: function (data, textStatus, XmlHttpRequest) {
                //Get the data values
                EntityData = data;
                //Process Result via callback function
                async_callback(EntityData);
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                if (asyncError_callback == null)
                    alert('Fail: Ajax Error in findViewbyName: ' + errorThrown);
                else
                    asyncError_callback(XMLHttpRequest, textStatus, errorThrown);
            },
            async: true,
            cache: false
        });
        ...
        //Display Error
        alert("An error occured in the findViewbyName function.  Error Detail Message: " + err);
    }
}

function findViewbyNameSync(EntitySetName, AttributeSet, ViewName) {
    //This function returns a CRM JSON dataset for a single entity record based on the View Name
    ...
    try {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, XmlHttpRequest) {
                //Get the data values
                EntityData = data.d.results;
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                alert('Fail: Ajax Error in findViewbyNameSync: ' + errorThrown);
            },
            async: false,
            cache: false
        });
        ...
        //Display Error
        alert("An error occured in the findViewbyNameSync function.  Error Detail Message: " + err);
    }
...
function ContainsEntities(req) {
    var returnValue = false;
    var resultXml = req.responseText;
    if (resultXml != null) {
        // Find a Name
        var testName = GetResponse(resultXml, "fullname");
